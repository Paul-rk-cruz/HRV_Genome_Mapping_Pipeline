...
RUN cd /usr/local/ && \
    ...
    conda install -c bioconda seqtk bowtie2 krakenuniq kallisto gmap snap-aligner samtools openssl bedtools bwa mafft bcftools tabix trimmomatic varscan vcftools bcftools seqkit bedtools fastqc ivar seqtk csvkit && \
    conda clean -afy
# Install Picard 
